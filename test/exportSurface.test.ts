...
// Owner-directed strip (2026-09-15): RC repair briefly expanded the public map
// with key-domain helpers. Those remain internal implementation; they must not
// reappear on the package export surface without a new owner ratification.
const STRIPPED_RC_EXPANSION = [
  "ED25519_KEY_DOMAIN_SEMANTICS",
  "verifyEd25519SpkiDetached",
];

test("RC public-export expansion remains stripped", () => {
  for (const name of STRIPPED_RC_EXPANSION) {
    assert.equal(
      name in publicApi,
      false,
      `${name} must not be publicly exported (owner-directed strip)`,
    );
  }
});
