...
test("Owner 1A: non-Ed25519 signer with Ed25519 pin is trust_key_type_unsupported", () => {
  const receipt = resignAsAttacker("rsa");
  const edPin = "MCowBQYDK2VwAyEA0EqyMnQrtKs6E2i9RhXk5tAiSrcaAWuvhSCjMsl3hzc=";
  const r = verifyReceiptV1(receipt, { now: VECTOR.now, trustedKeys: [edPin] });
  assert.equal(r.valid, false);
  assert.equal(r.keyTrusted, false);
