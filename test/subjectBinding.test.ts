...
//    keyTrusted:false + trust_config_missing, never an absent trust axis;
...
test("omitted trust policy: keyTrusted false + trust_config_missing, never absent", () => {
  for (const opts of [undefined, {}]) {
    ...
    assert.ok(r.reasons.includes("trust_config_missing"), JSON.stringify(r.reasons));
...
test("explicitly omitted options argument also fails closed and typed", () => {
  ...
  assert.equal(r.keyTrusted, false);
  assert.ok(r.reasons.includes("trust_config_missing"));
});
