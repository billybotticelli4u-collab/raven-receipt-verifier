...
export const verifyReceiptEvmV1 = (
  ...
  try {
    ...
    if (!pub) {
      ...
      // The receipt-evm-v1 signature algorithm is Ed25519. Building a key from
      // an untrusted SPKI and calling cryptoVerify(null, …) lets the key choose
      // the algorithm, so an attacker-chosen RSA/EC key verifies (ledgered
      // divergence D7 — the receipt-v1/D3 pattern on the EVM namespace). Reject
      // non-Ed25519 keys exactly like the Solana verifier and the Python
      // reference verifier ("not an Ed25519 SPKI key").
