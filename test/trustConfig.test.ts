...
// trust configuration and fails closed with `trust_config_missing`; only
// `allowUntrustedKey: true` is the typed opt-out. Non-object whole options
// are `trust_config_invalid` (Owner 2A). Non-Ed25519 pin members are
// `trust_config_invalid`; non-Ed25519 signers are `trust_key_type_unsupported`
// (Owner 1A). Exact Array/Set only (Owner 4A).
...
test("omitting trustedKeys and allowUntrustedKey fails closed on the trust axis", () => {
  ...
  assert.equal(r.keyTrusted, false);
  assert.deepEqual(r.reasons, ["trust_config_missing"]);
  assert.equal(r.valid, true);
});
...
test("trustedKeys + allowUntrustedKey:true is a contradiction (R7/R9)", () => {
  ...
  assert.equal(trusted.keyTrusted, false);
  assert.deepEqual(trusted.reasons, ["trust_config_invalid"]);

  const empty = verifyReceiptV1(wrongKey.input, {
    now: wrongKey.now,
    trustedKeys: [],
    allowUntrustedKey: true,
  });
  assert.equal(empty.valid, true);
  assert.equal(empty.keyTrusted, false);
  assert.deepEqual(empty.reasons, ["trust_config_invalid"]);
});
...
test("Owner 2A: non-object whole options are trust_config_invalid, not missing", () => {
  for (const [label, opts] of [
    ...
    const r = verifyReceiptV1(wrongKey.input, opts as never);
    assert.equal(r.valid, true, label);
    assert.equal(r.keyTrusted, false, label);
    assert.ok(hasOwn(r, "keyTrusted"), label);
    assert.ok(r.reasons.includes("trust_config_invalid"), label + ": " + JSON.stringify(r.reasons));
    assert.equal(r.reasons.includes("trust_config_missing"), false, label);
  }
});

test("Owner 2A/R1: undefined and empty-object options are trust_config_missing", () => {
  for (const [label, call] of [
    ["omitted", () => verifyReceiptV1(wrongKey.input)],
    ["undefined", () => verifyReceiptV1(wrongKey.input, undefined)],
    ["{}", () => verifyReceiptV1(wrongKey.input, {})],
    ["{now}", () => verifyReceiptV1(wrongKey.input, { now: wrongKey.now })],
  ] as const) {
    const r = call();
    assert.equal(r.valid, true, label);
    assert.equal(r.keyTrusted, false, label);
    assert.ok(hasOwn(r, "keyTrusted"), label);
    assert.ok(r.reasons.includes("trust_config_missing"), label + ": " + JSON.stringify(r.reasons));
    assert.equal(r.reasons.includes("trust_config_invalid"), false, label);
  }
});

test("R7/R9: trustedKeys together with allowUntrustedKey:true is trust_config_invalid", () => {
  const r7 = verifyReceiptV1(wrongKey.input, {
    now: wrongKey.now,
    trustedKeys: [GENUINE_KEY],
    allowUntrustedKey: true,
  });
  assert.equal(r7.keyTrusted, false);
  assert.deepEqual(r7.reasons, ["trust_config_invalid"]);

  const r9 = verifyReceiptV1(wrongKey.input, {
    now: wrongKey.now,
    trustedKeys: [],
    allowUntrustedKey: true,
  });
  assert.equal(r9.keyTrusted, false);
  assert.deepEqual(r9.reasons, ["trust_config_invalid"]);
});

test("Owner 4A: Array/Set subclasses are trust_config_invalid", () => {
  class KeyArray extends Array<string> {}
  class KeySet extends Set<string> {}
  const arr = new KeyArray();
  arr.push(GENUINE_KEY);
  const set = new KeySet([GENUINE_KEY]);
  for (const [label, keys] of [
    ["Array subclass", arr],
    ["Set subclass", set],
  ] as const) {
    const r = verifyReceiptV1(wrongKey.input, { now: wrongKey.now, trustedKeys: keys as never });
    assert.equal(r.keyTrusted, false, label);
    assert.deepEqual(r.reasons, ["trust_config_invalid"], label);
  }
});
