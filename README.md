...
The package name `raven-receipt-verifier` is ratified. It is **not yet published**
on the public npm registry (measured npm 404). There is no stranger-installable
canonical standalone verifier today. Do not install lookalike names.

Until an owner-authorized publish ceremony lands, obtain reviewed source through
an authenticated handoff. The public site (`security.html`, `integrate.html`)
already discloses this unpublished status. This README will carry the public
registry install command only after the package exists.

The package, when published, will export compiled ESM and TypeScript declarations from
...
`allowUntrustedKey: true`. Omitting both returns `keyTrusted: false` with
`trust_config_missing`; a trust policy that is present but malformed returns
`keyTrusted: false` with `trust_config_invalid`. The trust axis is never
silently absent.
...
6. **Key trust** *(non-fatal)* — is `signerPublicKey` an exact match for an
   ...
   be canonical Ed25519 SPKI. A non-Ed25519 key supplied in `trustedKeys` (for
   example P-256 or RSA material) is malformed trust configuration and fails
   closed with `trust_config_invalid`; a receipt whose *signer* key is
   non-Ed25519, checked against a well-formed Ed25519 trust set, reports
   `trust_key_type_unsupported`; a supported Ed25519 key that does not match
   the signer uses `key_untrusted`. Every other malformed value — a
   non-collection, a non-string member, or malformed key encoding — also fails
   typed with `keyTrusted: false` and `trust_config_invalid`, and no exception
   escapes. With no keys supplied, `allowUntrustedKey: true` is the typed
   opt-out (`key_trust_not_evaluated`) and `allowUntrustedKey: false` is a
   typed contract error (`trust_config_invalid`). Omitting both `trustedKeys`
   and `allowUntrustedKey` is missing trust configuration: it returns
   `keyTrusted: false` with `trust_config_missing`, never an absent axis. A
   non-boolean `allowUntrustedKey` — realistic for env-derived strings such as
   `"false"` — is malformed trust configuration and fails closed with
   `trust_config_invalid`; it never silently disables trust evaluation. Only
   *own* properties of the options object define trust configuration: values
   inherited through the prototype chain (including `Object.prototype`
   pollution) are treated as absent and their getters are never invoked.
...
### Trust-reason contract

The trust axis reports exactly one of these outcomes on every result. Each row
is enforced by the trust-contract corpus on all five verifier surfaces.

| Trust input | `keyTrusted` | Reason |
|---|---|---|
| Omitted trust policy (neither `trustedKeys` nor `allowUntrustedKey` supplied as an own property) | `false` | `trust_config_missing` |
| Present but malformed trust policy (non-object options, non-collection, non-string member, malformed key encoding, non-boolean flag, `allowUntrustedKey: false` alone, or `trustedKeys` together with `allowUntrustedKey: true`) | `false` | `trust_config_invalid` |
| Unsupported (non-Ed25519) trusted-key material in `trustedKeys` | `false` | `trust_config_invalid` |
| Unsupported (non-Ed25519) receipt signer, checked against a well-formed Ed25519 trust set | `false` | `trust_key_type_unsupported` |
| Well-formed Ed25519 trust set that does not contain the signer | `false` | `key_untrusted` |
| `allowUntrustedKey: true` with no `trustedKeys` (typed opt-out) | `false` | `key_trust_not_evaluated` |
| Exact match on an eligible Ed25519 key | `true` | — |
...
The fixtures under `fixtures/receipt-v1/` live in this source tree and are the
same golden vectors the in-app verifier is tested against — including a real
production-signed BONK receipt. They are **not** listed in package.json `files`
today, so a tarball packed from this manifest would not ship them. Whether the
published package includes fixtures is an Owner publish-time decision; this
README does not claim they are on the registry. The adjacent
