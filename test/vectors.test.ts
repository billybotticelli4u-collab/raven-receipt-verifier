...
const HOLDERS_PATH = "../../../apps/raven-solana-holders/holderAdmission.js";
const OUTCOME_PATH = "../../../apps/raven-solana-outcome/outcomeRules.js";
...
function loadMonorepoPeers():
  | { ok: true; validateSharedHolderAdmission: SharedAdmission["validateHolderAdmission"]; classifyCanonicalOutcome: CanonicalOutcome["classifyFindingEnvelopeOutcome"] }
  | { ok: false; reason: string } {
  try {
    const holders = require(HOLDERS_PATH) as SharedAdmission;
    const outcome = require(OUTCOME_PATH) as CanonicalOutcome;
    return {
      ok: true,
      validateSharedHolderAdmission: holders.validateHolderAdmission.bind(holders),
      classifyCanonicalOutcome: outcome.classifyFindingEnvelopeOutcome.bind(outcome),
    };
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    return { ok: false, reason: `monorepo peers absent (standalone public checkout): ${message}` };
  }
}
const MONOREPO_PEERS = loadMonorepoPeers();
...
test("1.1.4 outcome vectors match the canonical engine precedence exactly", (t) => {
  if (!MONOREPO_PEERS.ok) {
    t.skip(MONOREPO_PEERS.reason);
    return;
  }
  const { validateSharedHolderAdmission, classifyCanonicalOutcome } = MONOREPO_PEERS;
...
test("1.1.4 verifier reasons equal shared admission across bounded compound mutations", (t) => {
  if (!MONOREPO_PEERS.ok) {
    t.skip(MONOREPO_PEERS.reason);
    return;
  }
...
test("1.1.4 classifier metrics fail closed with one stable evidence reason", (t) => {
  if (!MONOREPO_PEERS.ok) {
    t.skip(MONOREPO_PEERS.reason);
    return;
  }
